...
title: 聊天室
uuid: d6761a69-efa5-4ad3-abab-1ba8 e6 c17 f94
...
聊天可讓您的觀眾參與即時活動的即時對話。內容會顯示為連續串流，鼓勵快速參與解除折疊對話框。

目前版本：聊天使用 `fyre.conv v3.0.0.`

## 整合 {#concept_0A99792A7E89403F95D082D52D600F17}

內嵌聊天應用程式會遵循「快速入門> [內嵌應用程式」中所述的核心應用程式程序](../c-getting-started/c-implementation-process/c-using-livefyre.js-to-create-customize-and-use-apps-on-your-site.md#using-livefyre-js-create-customize-apps)。
...
如 [CollectionMeta Token](../c-getting-started/c-implementation-process/c-collectionmeta-tokent.md#collectionmeta-tokent) 區段中所示，CollectionMeta是編碼的JSON物件。在上述範例中，JSON物件在JWT編碼之前採用下列格式：
...
| 網路 | *必要* 字串 | Livefyre提供的網路名稱。例如： *yourname. fyre. co.* |
| AttachmentDelegate | 物件 | 用來指定應用程式串流中可見的媒體附件類型。如需詳細資訊，請參閱 [限制媒體](../c-app-customizations/c-restrict-media.md#c_restrict_media)。 |
...
| CollectionMeta | *必要* 字串 | 關於系列的JWT編碼中繼資料。如需詳細資訊，請參閱 [CollectionMeta物件](../c-getting-started/c-implementation-process/c-collectionmeta-tokent.md#collectionmeta-tokent) 。 |
...
| checksum | *建議* 字串(建議) | 識別CollectionMeta的目前狀態。變更此值會使Livefyre在伺服器上更新資料與CollectionMeta中的新值。 |
...
| disableThirdPartyAnalytics | *選擇性* BBoolean | 停用Livefyre可用於內部測量的第三方分析系統(隨堂服務和Google Analytics)。預設為false。 |
...
如需詳細資訊，以及可用事件清單，請參閱 [Javascript事件](../c-app-customizations/c-javascript-events.md#c_javascript_events)。
