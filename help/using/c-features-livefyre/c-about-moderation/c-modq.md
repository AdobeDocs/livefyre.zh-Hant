...
* **[!UICONTROL History]** 按一 **[!UICONTROL History]** 下畫面右上方的按鈕，以開啟最近協調的內容清單，讓您檢閱作品或變更最近協調動作。 再按一下按鈕，返回佇列的內容。 只會顯示您最近的100個動作。 **歷史** ，不會列出其他協調者採取的動作。
...
按一 **[!UICONTROL Filter]** 下「ModQ」視窗左上角的，以開啟具有列出內容可用篩選選項的面板。 當您選取選項時，ModQ會自動更新，只列出您篩選的內容。 按一 **[!UICONTROL Clear filters]** 下以清除所有選取的選項，然後重新載入完整的項目清單。

和標籤有不同的篩 **[!UICONTROL Items]** 選選 **[!UICONTROL Streams Premoderation]** 項。
...
ModQ的下列選項可用於 **[!UICONTROL Streams Premoderation]**:
...
在和中設定內容時，會針對內容提供協調 **[!UICONTROL Network Settings > Moderation]** 建議 **[!UICONTROL Network Settings > ModQ]**。

## 可在ModQ中使用的動作 {#section_h4g_wrn_z1b}

您可以決定如何處理ModQ中的每段內容。

選擇下列選項之一：

* 要核 **[!UICONTROL Checkbox]** 準內容的圖示
* 將內 **[!UICONTROL Trash can]** 容傳送至垃圾筒的圖示
...
>[!NOTE]
>
>一旦您將內容廢棄後，內容片段及對內容的所有回覆都會從ModQ中永久移除。 若要將已銷毀的內容放入應用程式，請參 [閱「將已銷毀的項目新增至應用程式」](/help/using/c-features-livefyre/c-about-moderation/t-add-trashed-item-back-into-app.md#t_add_trashed_item_back_into_app)。
...
## 在ModQ中使用垃圾筒函式 {#section_tpx_qgz_y1b}

使用「設定」區段，選取當內容標示為「轉換」時可用的選項。

* ****[!UICONTROL Confirm Trash]**** 啟用此選項，要求協調者在將內容設定為垃圾筒時確認其動作。 啟用後，選 **[!UICONTROL Trash]** 擇內容時，會顯示要求輸入的對話 **[!UICONTROL Reason for Moderation]**&#x200B;方塊，並提供可輸入 **[!UICONTROL Note]** 內容的欄位。

   (此設定在網 **[!UICONTROL only]** 絡級別可用，並適用於您網路下的所有站點。)
